       ...
       console.log(self.donor_info);
        localService.createDonor(self.donor_info)
            .then(
                ...
                function (errResponse) {
                    console.error('Donor already exist in this mobile number');
...
/*    	 donors.createdonordate = today.getDate();
	...
	self.userdownload = function(){
		var downloadPath = 'http://localhost:8080/downloadnewdonor';
		window.open(downloadPath, '_blank','')
	};
	self.monthly_remittances = function(){
		var downloadPath = 'http://localhost:8080/downloaddonation';
		window.open(downloadPath, '_blank','')
	};
	...
	console.log('self.file', file);
	localService.uploadFile(file);
//	self.reload(); 
	} 
